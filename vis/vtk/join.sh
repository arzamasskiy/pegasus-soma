...
joinlist ()
{
  ...
  if [ ${xret} -ne 0 ]; then
    ...
    if [ ${notify} -ne 0 ]; then
      echo "${dir}/${outfile}" | mail -s "Error joining vtk file ${outfile}" $(whoami)@astro.princeton.edu
    fi
    exit ${xret}
  fi
  return ${xret}
...
while getopts "enwzd:f:b:i:o:p:x:" opt ; do
  case $opt in
    "e" )
...
if [ ${notify} -ne 0 ]; then
  if [ ${ret} -eq 0 ]; then
    echo "${dir}" | mail -s "Done joining, no errors: ${dir}" $(whoami)@astro.princeton.edu
  else
    echo "${dir}" | mail -s "Done joining, with errors: ${dir}" $(whoami)@astro.princeton.edu
  fi
fi
