 ...
 * MODIFIED BY: Matthew Kunz, May 8 2013
 *============================================================================*/
...
/* If argv[i] is a 2 character string of the form "-?" then: */
  ...
  for (i=f1; i<=f2; i+=fi)
  {
    ...
    for (p=0; p<nproc; p++)
    {
      ...
      fseek(fidin, 2*ntype*sizeof(float), SEEK_CUR);
    ...
    for (p=0; p<nproc; p++)
    {
      ...
      fread(&ntype,sizeof(int),1,fidin);
      fread(typeinfo,sizeof(float),2*ntype,fidin);
      ...
      if (p == 0)
      {
        ...
        fwrite(typeinfo,sizeof(float),2*ntype,fidout);
      ...
      for (j=0; j<n; j++)
      {
        fread(buffer,sizeof(float),10,fidin);
        fread(&property,sizeof(int),1,fidin);
        fread(&pid,sizeof(long),1,fidin);
        fread(&cpuid,sizeof(long),1,fidin);

        fwrite(buffer,sizeof(float),10,fidout);
        fwrite(&property,sizeof(int),1,fidout);
        fwrite(&pid,sizeof(long),1,fidout);
        fwrite(&cpuid,sizeof(long),1,fidout);
      }
