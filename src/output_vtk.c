...
/* write files if domain and level match input, or are not specified (-1) */
      if ((pOut->nlevel == -1 || pOut->nlevel == nl) &&
        ...
        if (pOut->ndim == 3) {
          ...
          peg_error("[output_vtk]: Only able to output 2D or 3D");
        }
      }}
    }
  }
 ...
 * <output> block.  */
  ...
  if((fname = peg_fname(plev,pM->outfilename,plev,pdom,num_digit,
    ...
    peg_error("[output_vtk]: Error constructing filename\n");
  }

/* open output file */
  if((pfile = fopen(fname,"w")) == NULL){
    peg_error("[output_vtk]: Unable to open vtk file %s\n",fname);
  }
...
/* Allocate memory for temporary array of floats */
  ...
  if((data = (float *)malloc(nx1*sizeof(float))) == NULL){
     peg_error("[output_vtk]: malloc failed for temporary array\n");
     return;
  }
...
/*  2. Header */
  ...
  fprintf(pfile,"Pegasus data at time= %e, level= %i, domain= %i\n",
    pGrid->time,nl,nd);
...
/* Write data */
  ...
  for (j=0; j<nx2; j++){
    ...
    if(!big_end) peg_bswap(data,sizeof(float),nx1);
...
/*  2. Header */
  ...
  fprintf(pfile,"Really cool Pegasus data at time= %e, level= %i, domain= %i\n",
    pGrid->time,nl,nd);
