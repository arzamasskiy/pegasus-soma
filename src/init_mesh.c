...
/* read levels of each domain block in input file and calculate max level */
  ...
  for (nblock=1; nblock<=num_domains; nblock++){
    ...
    if (par_exist(block,"level") == 0)
      peg_error("[init_mesh]: level does not exist in block %s\n",block);
...
/* set number of levels in Mesh, and allocate DomainsPerLevel array */
  ...
  if (pM->DomainsPerLevel == NULL)
    peg_error("[init_mesh]: malloc returned a NULL pointer\n");
...
/* Error if there are any levels with no domains.  Else set DomainsPerLevel */
    ...
    if (nd_this_level == 0) {
      peg_error("[init_mesh]: Level %d has zero domains\n",nl);
    } else {
 ...
 * and set root level properties in Mesh structure  */
  ...
  if (pM->DomainsPerLevel[0] != 1)
    peg_error("[init_mesh]: Level 0 has %d domains\n",pM->DomainsPerLevel[0]);
...
/* number of dimensions of root level, to test against all other inputs */
      ...
      if (nDim==0) peg_error("[init_mesh] None of Nx1,Nx2,Nx3 > 1\n");

/* some error tests of root grid */

      for (i=0; i<3; i++) {
        if (Nx[i] < 1) {
          peg_error("[init_mesh]: Nx%d in %s must be >= 1\n",(i+1),block);
        }
        if(root_xmax[i] < root_xmin[i]) {
          peg_error("[init_mesh]: x%dmax < x%dmin in %s\n",(i+1),block);
        }
      }
      if (nDim==1 && Nx[0]==1) {
        peg_error("[init_mesh]:1D requires Nx1>1: in %s Nx1=1,Nx2=%d,Nx3=%d\n",
        block,Nx[1],Nx[2]);
      }
      if (nDim==2 && Nx[2]>1) {peg_error(
 ...
 * Compare to the calloc_2d_array() function in peg_array.c
 */
      
  if((pM->Domain = (DomainS**)calloc((maxlevel+1),sizeof(DomainS*))) == NULL){
    peg_error("[init_mesh] failed to allocate memory for %d Domain pointers\n",
     (maxlevel+1));
  }

  if((pM->Domain[0]=(DomainS*)calloc(num_domains,sizeof(DomainS))) == NULL){
    peg_error("[init_mesh] failed to allocate memory for Domains\n");
  }
 ...
 * to the order it appears in input */
    ...
    if (next_domainid[nl] > (pM->DomainsPerLevel[nl])-1)
      peg_error("[init_mesh]: Exceeded available domain ids on level %d\n",nl);
    nd = next_domainid[nl];
    next_domainid[nl]++;
    irefine = 1;
...
/* error tests: dimensions of domain */
    ...
    if (nDim_test != nDim) {
      peg_error("[init_mesh]: in %s grid is %dD, but in root level it is %dD\n",
      block,nDim_test,nDim);
    }
    for (i=0; i<3; i++) {
      if (pM->Domain[nl][nd].Nx[i] < 1) {
        peg_error("[init_mesh]: %s/Nx%d = %d must be >= 1\n",
          block,(i+1),pM->Domain[nl][nd].Nx[i]);
      }
    }
    if (nDim==1 && pM->Domain[nl][nd].Nx[0]==1) {peg_error(
      "[init_mesh]: 1D requires Nx1>1 but in %s Nx1=1,Nx2=%d,Nx3=%d\n",
      block,pM->Domain[nl][nd].Nx[1],pM->Domain[nl][nd].Nx[2]);
    }
    if (nDim==2 && pM->Domain[nl][nd].Nx[2]>1) {peg_error(
    ...
    for (i=0; i<nDim; i++) {
      ...
      if (xdiv[i].rem != 0){
        peg_error("[init_mesh]: %s/Nx%d = %d must be divisible by %d\n",
 ...
 * displacement, so only read for levels other than root  */
    ...
    if (nl != 0) {  
      if (par_exist(block,"iDisp") == 0)
        peg_error("[init_mesh]: iDisp does not exist in block %s\n",block);
      pM->Domain[nl][nd].Disp[0] = par_geti(block,"iDisp");

/* jDisp=0 if problem is only 1D */
      if (pM->Nx[1] > 1) {
        if (par_exist(block,"jDisp") == 0)
          peg_error("[init_mesh]: jDisp does not exist in block %s\n",block);
...
/* kDisp=0 if problem is only 2D */
      if (pM->Nx[2] > 1) {
        if (par_exist(block,"kDisp") == 0)
          peg_error("[init_mesh]: kDisp does not exist in block %s\n",block);
    ...
    for (i=0; i<nDim; i++) {
      ...
      if (xdiv[i].rem != 0){
        peg_error("[init_mesh]: %s/Disp%d = %d must be divisible by %d\n",
  ...
  for (nd=0; nd<(pM->DomainsPerLevel[nl])-1; nd++){
    ...
    for (ncd=nd+1; ncd<(pM->DomainsPerLevel[nl]); ncd++) {
      ...
      if (D1.ijkl[0] <= D2.ijkr[0] && D1.ijkr[0] >= D2.ijkl[0] &&
          ...
          peg_error("Domains %d and %d at same level overlap or touch\n",
 ...
 * root), extends past edge of parent, or are < nghost/2 from edge of parent  */
        ...
        for (dim=0; dim<nDim; dim++){
          ...
          if (((D2.ijkl[dim] == D1.ijkl[dim]) && (pD->Disp[dim] != 0)) ||
            ...
            peg_error("[init_mesh] child Domain D%d[is,ie,js,je,ks,ke]=[%d %d %d %d %d %d] touches parent D%d[is,ie,js,je,ks,ke]=[%d %d %d %d %d %d]\n",
          ...
          if ((D2.ijkl[dim] < D1.ijkl[dim]) ||
            ...
            peg_error("[init_mesh] child Domain D%d[is,ie,js,je,ks,ke]=[%d %d %d %d %d %d] extends past parent D%d[is,ie,js,je,ks,ke]=[%d %d %d %d %d %d]\n",
              pCD->InputBlock,D2.ijkl[0],D2.ijkr[0],D2.ijkl[1],D2.ijkr[1],
              D2.ijkl[2],D2.ijkr[2],pD->InputBlock,D1.ijkl[0],D1.ijkr[0],
              D1.ijkl[1],D1.ijkr[1],D1.ijkl[2],D1.ijkr[2]);
          }
          ...
          if (((2*(D2.ijkl[dim]-D1.ijkl[dim]) < nghost) &&
            ...
            peg_error("[init_mesh] child Domain D%d[is,ie,js,je,ks,ke]=[%d %d %d %d %d %d] closer than nghost/2 to parent D%d[is,ie,js,je,ks,ke]=[%d %d %d %d %d %d]\n",
              pCD->InputBlock,D2.ijkl[0],D2.ijkr[0],D2.ijkl[1],D2.ijkr[1],
              D2.ijkl[2],D2.ijkr[2],pD->InputBlock,D1.ijkl[0],D1.ijkr[0],
              D1.ijkl[1],D1.ijkr[1],D1.ijkl[2],D1.ijkr[2]);
          }
...
/* Read layout of Grids from input file */
      ...
      if (nproc == 0){
        ...
        if (pD->NGrid[0] == 0)
          peg_error("[init_mesh] Cannot enter NGrid_x1=0 in %s\n",block);
        if (pD->NGrid[1] == 0)
          peg_error("[init_mesh] Cannot enter NGrid_x2=0 in %s\n",block);
        if (pD->NGrid[2] == 0)
          peg_error("[init_mesh] Cannot enter NGrid_x3=0 in %s\n",block);
      }
 ...
 * to number of processors desired for this Domain  */
        ...
        if(dom_decomp(pD->Nx[0],pD->Nx[1],pD->Nx[2],nproc,
           ...
           peg_error("[init_mesh]: Error in automatic Domain decomposition\n");
        ...
        peg_error("[init_mesh] invalid AutoWithNProc=%d in %s\n",nproc,block);
      }
#endif /* MPI_PARALLEL */
...
/* test for conflicts between number of grids and dimensionality */
      ...
      for (i=0; i<3; i++){
        if(pD->NGrid[i] > 1 && pD->Nx[i] <= 1)
          peg_error("[init_mesh]: %s/NGrid_x%d = %d and Nx%d = %d\n",block,
...
/* check there are more processors than Grids needed by this Domain. */
      ...
      if(nproc > Nproc_Comm_world) peg_error(
...
/* Build 3D array to store data on Grids in this Domain */
      ...
      if ((pD->GData = (GridsDataS***)calloc_3d_array(pD->NGrid[2],pD->NGrid[1],
        pD->NGrid[0],sizeof(GridsDataS))) == NULL) peg_error(
 ...
 * MPI processes available (equal to one for single processor jobs) */ 
  ...
  if (next_procID != 0)
    peg_error("[init_mesh]:total # of Grids != total # of MPI procs\n");
      ...
      for(l=0; l<(pD->NGrid[0]); l++){
        if (pD->GData[n][m][l].ID_Comm_world == myID_Comm_world) {
          if ((pD->Grid = (GridS*)malloc(sizeof(GridS))) == NULL)
            peg_error("[init_mesh]: Failed to malloc a Grid for %s\n",block);
        }
      }}}
    }
  }
                     ...
                     int *pi, int *pj, int *pk)
{
  ...
  peg_error("[get_myGridIndex]: Can't find ID=%i in GData\n", myID);
}
