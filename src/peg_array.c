...
void* calloc_1d_array(size_t nc, size_t size)
{
  ...
  if ((array = (void *)calloc(nc,size)) == NULL) {
    peg_error("[calloc_1d] failed to allocate memory (%d of size %d)\n",
              (int)nc,(int)size);
    return NULL;
  }
  return array;
...
void** calloc_2d_array(size_t nr, size_t nc, size_t size)
{
  ...
  if((array = (void **)calloc(nr,sizeof(void*))) == NULL){
    peg_error("[calloc_2d] failed to allocate mem for %d pointers\n",(int)nr);
    return NULL;
  }

  if((array[0] = (void *)calloc(nr*nc,size)) == NULL){
    peg_error("[calloc_2d] failed to allocate memory (%d X %d of size %d)\n",
...
void*** calloc_3d_array(size_t nt, size_t nr, size_t nc, size_t size)
{
  ...
  if((array = (void ***)calloc(nt,sizeof(void**))) == NULL){
    peg_error("[calloc_3d] failed to allocate memory for %d 1st-pointers\n",
              (int)nt);
    return NULL;
  }

  if((array[0] = (void **)calloc(nt*nr,sizeof(void*))) == NULL){
    peg_error("[calloc_3d] failed to allocate memory for %d 2nd-pointers\n",
              (int)(nt*nr));
    free((void *)array);
    return NULL;
  }
  ...
  if((array[0][0] = (void *)calloc(nt*nr*nc,size)) == NULL){
    peg_error("[calloc_3d] failed to alloc. memory (%d X %d X %d of size %d)\n",
