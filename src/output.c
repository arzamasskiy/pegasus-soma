 ...
 * - out       = all, fields (B and E), or moments (d,M1,M2,M3,P11,P12,P13,P22,P23,P33)
 * - out_fmt   = bin,hst,phst,lis,tab,rst,vtk,pdf,pgm,ppm
 ...
 *   
 * EXAMPLE of an <outputN> block for a VTK dump of electromagnetic fields:
 * - <output1>
 * - out_fmt = vtk
 * - out     = fld
 * - out_dt  = 0.1
...
/* allocate output array */
  ...
  if((OutArray = malloc(maxout*sizeof(OutputS))) == NULL){
    peg_error("[init_output]: Error allocating output array\n");
  }
 ...
 * If neither is present we write an error message and move on. */
    if((par_exist(block,"out_fmt") == 0) && (par_exist(block,"name") == 0)){
      peg_perr(-1,"[init_output]: neither %s/out_fmt, nor %s/name exist\n",
	       block, block);
      continue;
    }
...
/* Check to see if either dt or dcycle are defined. Do not move on otherwise. */
   if((par_exist(block,"dt") == 0) && (par_exist(block,"dcycle") == 0)){
      peg_perr(-1,"[init_output]: neither %s/dt, nor %s/dcycle exist\n",
	       block, block);
  	  exit(EXIT_FAILURE);
	  continue;
   }
...
/* The next output time and number */
    ...
    new_out.dt     = par_getd_def(block,"dt",-1);
    new_out.cycle  = par_geti_def(block,"cycle",pM->nstep);
    new_out.dcycle = par_geti_def(block,"dcycle",-1);
...
/* out:     controls what variable can be output (all, fld, mom, or any of expr_*)
 ...
 * if "out" doesn't exist, we assume 'cons' variables are meant to be dumped */
    ...
    if ((new_out.out_pargrid < 0) || (new_out.out_pargrid >1)) {
      peg_perr(-1,"[init_output]: %s/pargrid must be 0 or 1\n", block);
      continue;
    }
 ...
 * particles. Used only when particle output is called, otherwise useless. */
    if(par_exist(block,"par_prop")) {
      ...
      if (new_out.par_prop == NULL) {
        peg_pout(0,"[init_output]: Particle selection function not found! \
...
/* First handle data dumps of all variables (out=all) */

    if(strcmp(new_out.out,"all") == 0){
/* check for valid data dump: dump format = {bin, hst, phst, lis, tab, rst, vtk} */
	...
	/* Get a pointer to the output function via its name */
        ...
        if(new_out.out_fun == NULL){
          free_output(&new_out);
          peg_error("Unsupported output named %s in %s/out_fmt=%s\n",
                    name,block,fmt);
        }
        free(name);  name = NULL;
        goto add_it;
        ...
        rst_flag = 1;
        rst_out = new_out;
        peg_pout(0,"Added out%d\n",outn);
        continue;
      }
      else if (strcmp(fmt,"vtk")==0){
        sprintf(new_out.id,"all");
        ...
        new_out.out_pargrid = 0;
        goto add_it; /* by default do not bin particles */
      }
      else if (strcmp(fmt,"spec")==0){
        new_out.out_fun = dump_particle_spectrum;
        new_out.out_pargrid = 0;
        goto add_it; /* by default do not bin particles */
      }
      else if (strcmp(fmt,"track")==0){ /* dump particle tracks */
        new_out.out_fun = dump_particle_track;
        new_out.out_pargrid = 0;
        goto add_it; /* by default do not bin particles */
      } 
      else{    /* Unknown data dump (fatal error) */
        peg_error("Unsupported dump mode for %s/out_fmt=%s for out=all\n",
                  block,fmt);
      }
    }
    
/* Next handle data dumps of only electromagnetic fields (out=field) */
    
    if(strcmp(new_out.out,"field") == 0){
      sprintf(new_out.id,"fld");
      ...
      if(par_exist(block,"name")){
        ...
        peg_error("Unsupported dump mode for %s/out_fmt=%s for out=field\n",
                  block,fmt);
      }
    }

/* Next handle data dumps of only moments of distribution function (out=moment) */
    
    if(strcmp(new_out.out,"moment") == 0){
      sprintf(new_out.id,"mom");
      /* check for valid data dump: dump format = {bin, tab, vtk} */
      if(par_exist(block,"name")){
        /* The output function is user defined - get its name */
        char *name = par_gets(block,"name");
        ...
        new_out.out_fun = dump_vtk;
        new_out.out_pargrid = 1;
        goto add_it;
      }
      else{    /* Unknown data dump (fatal error) */
        peg_error("Unsupported dump mode for %s/out_fmt=%s for out=moment\n",
                  block,fmt);
      }
    }
...
/* Get the expression function pointer */
    ...
    if (new_out.expr == NULL) {
      peg_perr(-1,"Could not parse expression %s, skipping it\n",
	      new_out.out);
      free_output(&new_out);
      continue;
    }
...
/* x1, x2, x3:  parse coordinate range for slicing and averaging */
    ...
    if (new_out.ndim <= 0) peg_error("Too many slices specified in %s\n",block);
...
/* unknown output format is fatal */
      free_output(&new_out);
      peg_error("Unsupported %s/out_fmt=%s\n",block,fmt);
    }
...
/* Now copy data in "new_out" into OutArray structure, and increment index. */
    ...
    peg_pout(1,"OUTPUT: %d %d %s %s %s [%g : %g]\n",
             new_out.n, new_out.ndim, new_out.out_fmt,
             new_out.out, new_out.id, new_out.dmin, new_out.dmax); /* DEBUG */
 ...
 * set dump flag to input argument, check whether time for output */
  ...
  for (n=0; n<out_count; n++) {
    dump_flag[n] = flag;
    if (OutArray[n].dt > 0 && pM->time >= OutArray[n].t) {
      OutArray[n].t += OutArray[n].dt;
      dump_flag[n] = 1;
    }
   
    if (OutArray[n].dcycle > 0 && pM->nstep >= OutArray[n].cycle) {
      OutArray[n].cycle += OutArray[n].dcycle;
      dump_flag[n] = 1;
    } 
  }
...
/* Now check for restart dump, and make restart if dump_flag != 0 */
  ...
  if(rst_flag){
    ...
    if(rst_out.dt > 0 && pM->time >= rst_out.t){
      rst_out.t += rst_out.dt;
      dump_flag[out_count] = 1;
    } 

    if(rst_out.dcycle > 0 && pM->nstep >= rst_out.cycle){
      rst_out.cycle += rst_out.dcycle;
      dump_flag[out_count] = 1;
    } 
 ...
 * number given in the restart file */
		...
		} else if(OutArray[n].dcycle > 0){
			par_seti(block,"cycle","%d",OutArray[n].cycle,"Next Output Cycle");
		} else {
    		peg_perr(-1,"[Restart]: Cadence not set properly for Out%d\n",n);
        }
	}
  }
/* Now do the same for the restart output block */
	  ...
	  } else if(rst_out.dcycle > 0){
	      par_seti(block,"cycle","%d",rst_out.cycle,"Next Output Cycle");
      } else {
       	  peg_perr(-1,"[Restart]: Cadence not set properly for restart block\n");
      }
...
/* print the global min/max computed over the calculation */
    ...
    if (OutArray[i].out != NULL){
      if((strcmp(OutArray[i].out,"all") != 0) &&
         (strcmp(OutArray[i].out,"field") != 0) &&
         (strcmp(OutArray[i].out,"moment") != 0)){
...
#endif
	peg_pout(0,"Global min/max for %s: %g %g\n",OutArray[i].out,
...
Real ***OutData3(GridS *pgrid, OutputS *pout, int *Nx1, int *Nx2, int *Nx3)
{
  ...
  if (pout->ndim != 3) peg_error("[OutData3] <output%d> %s is %d-D, not 3-D\n",
  ...
  *Nx3 = ku-kl+1;
  ...
  if (data == NULL) peg_error("[OutData3] Error creating 3D data array\n");
  for (k=0; k<*Nx3; k++)
    for (j=0; j<*Nx2; j++)
      for (i=0; i<*Nx1; i++)
...
Real **OutData2(GridS *pgrid, OutputS *pout, int *Nx1, int *Nx2)
{
  ...
  if (pout->ndim != 2) peg_error("[OutData2] <output%d> %s is %d-D, not 2-D\n",
    pout->n,pout->out, pout->ndim);
...
/* data is already 2D in 2D simulations */
  ...
  if (pgrid->Nx[2] == 1) {
    ...
    if (data == NULL) peg_error("[OutData2] Error creating 2D data array\n");
    for (j=0; j<*Nx2; j++) {
      for (i=0; i<*Nx1; i++) {
...
/* Nx3,Nx2,Nx1 -> Nx3,Nx2 */
    ...
    *Nx2 = Nx3; /* return dimensions of array created */
    ...
    peg_perr(-1,"[OutData2]: Should not reach here\n");
  return data;
}
...
Real *OutData1(GridS *pgrid, OutputS *pout, int *Nx1)
{
  ...
  if (pout->ndim != 1) peg_error("[OutData1] <output%d> %s is %d-D, not 1-D\n",
    pout->n,pout->out, pout->ndim);
...
/* data is already 1D in 1D simulations */
  ...
  if (pgrid->Nx[1] == 1) {
    ...
    if (data == NULL) peg_error("[OutData1] Error creating 1D data array\n");
    for (i=0; i<*Nx1; i++) {
...
/* Nx3,Nx2,Nx1 -> Nx3. Data must be 3D in this case. */
    ...
    *Nx1 = Nx3; /* return dimensions of array created */
    ...
    peg_perr(-1,"[OutData1]: Should not reach here\n");
  }
...
static ConsFun_t getexpr(const int n, const char *expr)
{
  ...
  if (strcmp(expr,"B1c")==0)
    ...
    peg_perr(-1,"Unknown data expression\n");
    return NULL;
  }
}
...
static void parse_slice(char *block, char *axname, Real *l, Real *u, int *flag)
{
  ...
  if (par_exist(block,axname)) {
	...
	*l = atof(cp);
    ...
    if (*l > *u) {
      peg_error("[parse_slice]: lower slice limit %d > upper %d in %s\n",
      *l,*u,expr);
    }
    free(expr);
    *flag = 1;
...
/* failed to find a matching palette: print them all and exit...  */
  ...
  peg_perr(-1,"Fatal error: could not find palette=%s, valid names are:\n",
    name);
  for (i=0; rgb[i].name && rgb[i].rgb; i++)
    peg_perr(-1,"%s ",rgb[i].name);
  peg_perr(-1,"\n");
  exit(EXIT_FAILURE);
