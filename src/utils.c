 ...
 * - peg_strdup()     - not supplied by fancy ANSI C, but ok in C89 
 * - peg_gcd()        - computes greatest common divisor by Euler's method
 * - peg_big_endian() - run-time detection of endianism of the host cpu
 * - peg_bswap()      - fast byte swapping routine
 * - peg_error()      - fatal error routine
...
/*----------------------------------------------------------------------------*/
/*! \fn char *peg_strdup(const char *in)
...
char *peg_strdup(const char *in)
{
  ...
  if(out == NULL) {
    peg_perr(-1,"peg_strdup: failed to alloc %d\n",(int)(1+strlen(in)));
...
/*----------------------------------------------------------------------------*/
/*! \fn void peg_bswap(void *vdat, int len, int cnt)
 *  \brief Swap bytes, code stolen from NEMO  
 */
 
void peg_bswap(void *vdat, int len, int cnt)
{
...
{
  va_list ap;
   FILE *pegerr = pegerr_fp();

  fprintf(pegerr,"### Fatal error: ");   /* prefix */
  va_start(ap, fmt);              /* ap starts with string 'fmt' */
  vfprintf(pegerr, fmt, ap);      /* print out on pegerr */
  fflush(pegerr);                 /* flush it NOW */
 ...
 * grids.  Begin by copying the error into the err[] array */
  ...
  err[0] = total_error.B1c;
  err[1] = total_error.B2c;
  err[2] = total_error.B3c;

/* Sum up the Computed Error */
  mpi_err = MPI_Reduce(err, tot_err, 3,
                       MPI_DOUBLE, MPI_SUM, 0, pD->Comm_Domain);
  if(mpi_err)
    peg_error("[compute_l1_error]: MPI_Reduce call returned error = %d\n",
              mpi_err);

/* If I'm the parent, copy the sum back to the total_error variable */
  if(pD->DomNumber == 0){ /* I'm the parent */
    total_error.B1c = tot_err[0];
    total_error.B2c = tot_err[1];
    total_error.B3c = tot_err[2];
  }
...
/* Print error to file "BLAH-errors.#.dat"  */
   ...
   fname = peg_fname(NULL,fnamestr,NULL,NULL,1,0,NULL,"dat");

/* The file exists -- reopen the file in append mode */
  if((fp=fopen(fname,"r")) != NULL){
    if((fp = freopen(fname,"a",fp)) == NULL){
      peg_error("[compute_l1_error]: Unable to reopen file.\n");
      free(fname);
      return;
    }
  }
/* The file does not exist -- open the file in write mode */
    ...
    if((fp = fopen(fname,"w")) == NULL){
      peg_error("[compute_l1_error]: Unable to open file.\n");
      free(fname);
      return;
    }
/* Now write out some header information */
    fprintf(fp,"# Nx1  Nx2  Nx3  RMS-Error");
...
//   peg_error("[sign_change]: No sign change was detected in (%f,%f) for x=%f!\n",a0,b0,x);
  return 0;
} 
...
// printf("x = %f, a = %f, b = %f, c = %f\n", x,a,b,c);
    ...
    if (fa*fc < 0) {
      ...
      *root = c;
      ...
      peg_error("[bisection]:  There is no single root in (%f,%f) for x = %13.10f!!\n", a, b,x);
      *root = c;
      return 0;
    }
  }

  peg_error("[bisection]:  Bisection did not converge in %d iterations for x = %13.10f!!\n", maxiter,x);
  *root = c;
  return 0;
}
...
Real qsimp(Real (*func)(Real), const Real a, const Real b) 
{
  ...
  peg_error("[qsimp]:  Too many steps!\n");
  return 0.0;
}
                ...
                const GridS *pG, const int i, const int j, const int k)
{
  ...
  if (A2 != NULL) {
    if (ymin == ymax)
      b1i += A2(xmin,ymin,zmin) - A2(xmin,ymin,zmax);
    else {
      zsav = zmin;
      b1i += qsimp(f2,ymin,ymax);
      zsav = zmax;
      b1i -= qsimp(f2,ymin,ymax);
    }
  }
                ...
                const GridS *pG, const int i, const int j, const int k)
{
  ...
  if (A2 != NULL) {
    if (ymin == ymax)
      b3i += A2(xmax,ymin,zmin) - A2(xmin,ymin,zmin);
    else {
      xsav = xmax;
      b3i += qsimp(f2,ymin,ymax);
      xsav = xmin;
      b3i -= qsimp(f2,ymin,ymax);
    }
  }
...
void ludcmp(Real **a, int n, int *indx, Real *d)
{
  ...
  *d=1.0;  /* No row interchanges yet */
  ...
  { /* Loop over rows to get the implicit scaling information */
    ...
    if (big == 0.0) peg_error("[LUdecomp]:Input matrix is singular!");
