 ...
 * - 1-mar-2004   Moved error out to utils.c as peg_error()         -- PJT
...
void par_open(char *filename)
{
  ...
  if (now_open) peg_error("Parameter file %s still open\n",now_filename);
  if (now_filename) free(now_filename);
  fp = fopen(filename,"r");
  if (fp == NULL) 
    peg_error("Parameter file %s could not be opened, try -i PARFILE\n"
      ,filename);
...
void par_cmdline(int argc, char *argv[])
{
  ...
  for (i=1; i<argc; i++) {
    ...
    *ep = '\0';
    ...
    bp = find_block(block);
    if (bp == NULL) peg_error("par_cmdline: Block \"%s\" not found\n",block);
    pp = find_par(bp,name);
    if (pp == NULL) peg_error("par_cmdline: Par \"%s\" not found\n",name);
    free(pp->value);
...
int par_exist(char *block, char *name)
{
  ...
  if (!now_open) peg_error("par_exist: No open parameter file\n");
  if (block == NULL) peg_error("par_exist: no block name specified\n");
  if (name  == NULL) peg_error("par_exist: no par name specified\n");
  bp = find_block(block);
  if (bp == NULL) return 0;
  pp = find_par(bp,name);
  return (pp == NULL ? 0 : 1);
...
void par_dist_mpi(const int mytid, MPI_Comm comm)
{
  ...
  if(mytid == 0){    /* I'm the rank=0 process */
    if (!now_open) peg_error("Parameter file is not open\n");
    if(debug)
...
/* Share the filename with the children */
    ...
    if(MPI_SUCCESS != MPI_Bcast(&count, 1, MPI_INT, 0, comm))
      peg_error("[par_dist_mpi]: Error on calling MPI_Bcast\n");
...
/* Broadcast the "name = value # comment" string */
    ...
    if (now_open) peg_error("Parameter %s file is already open\n",now_filename);
...
static void *allocate(size_t size) 
{
  ...
  if(mem == NULL) peg_error("allocate: not enough memory for %d bytes\n",size);
  return mem;
}
...
static char *my_strdup(char *in)
{
  char *out = peg_strdup(in);
  if(out == NULL) peg_error("Error allocating memory for a new string\n");
  return out;
}
...
/* Skip leading white space and remember the start of block name */
  ...
  while (*cp != '\0' && *cp != '>')
    cp++;
  if (*cp != '>') peg_error("Blockname %s does not appear terminated\n",sp);
...
static void add_par_line(Block *bp, char *line)
{
  ...
  if(bp == NULL)
    peg_error("[add_par_line]: (no block name) while parsing line \n%s\n",line);
...
static char *par_getsl(char *block, char *name)
{
  ...
  if (!now_open) peg_error("par_gets: No open parameter file\n");
  if (block == NULL) peg_error("par_gets: no block name specified\n");
  if (name  == NULL) peg_error("par_gets: no par name specified\n");
  bp = find_block(block);
  if (bp == NULL) peg_error("par_gets: Block \"%s\" not found\n",block);
  pp = find_par(bp,name);
  if (pp == NULL)
    peg_error("par_gets: Par \"%s\" not found in Block \"%s\"\n",name,block);
  return pp->value;
}
...
/* int numprocs; */
  ...
  if(MPI_SUCCESS != MPI_Init(&argc,&argv))
    peg_error("Error on calling MPI_Init\n");
