...
void problem(DomainS *pDomain)
{
  ...
  Real amp,vflow,b0,lshear,sigma,x1,x2,x3;
  long int iseed = -1;
  ...
  for (j=js; j<=je; j++)
	{
      ...
      {        
        ...
        {         
          ...
          dv1  = 0.5*vflow*(tanh((rx2-Ly/4.0)/lshear) - tanh((rx2-3.0*Ly/4.0)/lshear) - 1.0);
          dv2  = amp*sin(2.0*PI*rx1/Lx)*exp(-SQR(rx2-0.5*Ly)/(Ly*Ly*sigma*sigma));
          dv3  = 0.0;
