 ...
 * caution!
 *
 * These functions were written for Athena by T. A. Gardiner
 * in March, 2008.
 *
...
void baton_start(const int Nb, const int tag){
  ...
  if(status != MPI_SUCCESS)
    peg_error("[baton_start]: MPI_Comm_rank error = %d\n",status);
  ...
  if(Nb > 0 && fr_id >= 0){
    ...
    if(status != MPI_SUCCESS)
      peg_error("[baton_start]: MPI_Recv error = %d\n",status);
  }
...
void baton_stop(const int Nb, const int tag){
  ...
  status = MPI_Comm_rank(MPI_COMM_WORLD, &my_id);
  if(status != MPI_SUCCESS)
    peg_error("[baton_stop]: MPI_Comm_rank error = %d\n",status);

  status = MPI_Comm_size(MPI_COMM_WORLD, &nproc);
  if(status != MPI_SUCCESS)
    peg_error("[baton_stop]: MPI_Comm_size error = %d\n",status);
  ...
  if(Nb > 0 && to_id < nproc){
    ...
    if(status != MPI_SUCCESS)
      peg_error("[baton_stop]: MPI_Send error = %d\n",status);
  }
